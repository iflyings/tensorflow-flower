...
#创建自定义分类器
classifier = tf.estimator.Estimator(model_fn=my_model_fn,model_dir='./model',
        params={
...
#开始训练
train_op = classifier.train(input_fn=lambda:train_input_fn(train_x, train_y, 100), steps=1000)
#进行预测
predict_op = classifier.predict(input_fn=lambda:eval_input_fn(predict_x, labels=[0,], batch_size=batch_size))
#评估我们训练出来的模型质量
evaluate_op = classifier.evaluate(input_fn=lambda:eval_input_fn(test_x, test_y,batch_size))
